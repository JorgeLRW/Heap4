...
try {
  console.log('--- 1. Testing the reproducible application defect ---');
  execSync('npx tsx tests/deliveryService.test.ts', { stdio: 'inherit' });

  console.log('\n--- 2. Testing Intent Creation & Server Context ---');
  execSync('npx tsx tests/intent.test.ts', { stdio: 'inherit' });

  console.log('\n--- 3. Testing Invariant Guard & Resume Lifecycle ---');
